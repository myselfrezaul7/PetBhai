...
router.delete(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (String(post.comments[commentIndex].author.id) !== userId) {
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!validatedAuthor || validatedAuthor.id !== requesterId) {
        return res
          .status(400)
          .json({ message: 'Valid author information is required and must match user' });
      }
...
router.put(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (String(reply.author.id) !== userId) {
...
router.delete(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (String(comment.replies[replyIndex].author.id) !== userId) {
