  ...
  return (
    <Link to={`/vet/${vet.id}`} className="glass-card flex flex-col text-center items-center p-3 sm:p-6 group transition-transform transform hover:-translate-y-2 h-full">
      <div className="relative">
        <img className="w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover ring-4 ring-orange-500/20" src={vet.imageUrl} alt={`Dr. ${vet.name}`} loading="lazy" />
        <span className={`absolute bottom-0 right-0 block h-max w-max px-2 py-0.5 border-2 border-white dark:border-slate-800 rounded-full text-[10px] sm:text-xs font-bold ${availabilityStyles[vet.availability]}`}>
            {vet.availability === 'Available Now' ? 'Now' : (vet.availability === 'Available Today' ? 'Today' : 'Offline')}
        </span>
      </div>
      <h3 className="text-sm sm:text-xl font-bold text-slate-800 dark:text-white mt-3 sm:mt-4 group-hover:text-orange-500 transition-colors line-clamp-1">{vet.name}</h3>
      <p className="text-xs sm:text-base text-orange-600 dark:text-orange-400 font-semibold line-clamp-1">{vet.specialization}</p>
      
      <div className="flex items-center justify-center space-x-2 sm:space-x-4 mt-2 sm:mt-4 text-slate-600 dark:text-slate-300">
        {vet.services.some(s => s.type === 'in-clinic') && (
            <div className="flex items-center space-x-1 text-[10px] sm:text-sm">
                ...
                <span className="hidden sm:inline">In-Clinic</span>
            </div>
        )}
      ...
      <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 mt-2 sm:mt-4 flex-grow line-clamp-2">{vet.bio}</p>

      <div className="mt-3 sm:mt-4 w-full pt-3 sm:pt-4 border-t border-white/20 dark:border-slate-700/50">
          <span
            className="w-full block bg-orange-500 text-white font-bold py-2 sm:py-2.5 px-4 rounded-lg text-xs sm:text-base group-hover:bg-orange-600 transition-colors"
          >
            Profile
          </span>
      </div>
