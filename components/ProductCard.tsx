  ...
  return (
    <div className="glass-card group overflow-hidden flex flex-col h-full transform transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl relative">
      <Link to={`/product/${product.id}`} className="relative block overflow-hidden aspect-[4/3] sm:aspect-square">
        ...
        {/* Quick View Button - Always visible on mobile for easier access, hover on desktop */}
        {onQuickView && (
            <button
        ...
        {/* In Cart Badge */}
        {quantityInCart > 0 && (
            <div className="absolute bottom-2 left-2 bg-green-600/95 backdrop-blur-md text-white text-[10px] sm:text-xs font-bold px-2 py-1 rounded-md shadow-lg z-10 flex items-center gap-1 animate-scale-in">
      ...
      <div className="p-3 sm:p-5 flex flex-col flex-grow">
        <Link to={`/product/${product.id}`} className="block group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors duration-200">
             <h3 className="text-sm sm:text-lg font-bold text-slate-800 dark:text-white line-clamp-2 leading-tight min-h-[2.5em]" title={product.name}>{product.name}</h3>
        </Link>
        ...
        <div className="flex justify-between items-end mt-2 sm:mt-4 mb-1 sm:mb-3">
          <p className="text-base sm:text-xl font-extrabold text-slate-800 dark:text-white">
      ...
      <div className="p-3 sm:p-4 bg-white/50 dark:bg-black/20 mt-auto flex flex-col sm:flex-row gap-2 sm:gap-3 border-t border-white/20 dark:border-slate-700/30">
        <button 
            ...
            disabled={isAdding}
            className={`flex-1 flex items-center justify-center space-x-1.5 sm:space-x-2 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 
            ${isAdding 
