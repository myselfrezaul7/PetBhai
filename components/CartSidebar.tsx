...
const CartSidebar: React.FC<CartSidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-slate-50 dark:bg-slate-900 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <header className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-800 dark:hover:text-white"
          ...
          <div className="flex-grow overflow-y-auto p-4">
            {cartItems.length === 0 ? (
              ...
              <ul className="space-y-4">
                {cartItems.map(item => (
                  <li key={item.id} className="flex items-center space-x-4 bg-white dark:bg-slate-800 p-3 rounded-lg shadow-sm">
          ...
          {/* Footer */}
          {cartItems.length > 0 && (
            <footer className="p-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50">
