  ...
  return (
    <div className="glass-card group overflow-hidden flex flex-col transform transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl h-full">
      <div className="relative overflow-hidden aspect-[4/3]">
        ...
        <div className={`absolute top-2 left-2 sm:top-3 sm:left-3 px-1.5 py-0.5 sm:px-3 sm:py-1 text-[9px] sm:text-xs font-bold rounded-full backdrop-blur-md shadow-sm ${statusColors[animal.status]}`}>
            {animal.status}
        </div>
         <button 
      ...
      <div className="p-3 sm:p-5 flex flex-col flex-grow">
        <div className="flex justify-between items-start">
            <div>
                <h3 className="text-sm sm:text-2xl font-bold text-slate-800 dark:text-white group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">{animal.name}</h3>
                <p className="text-slate-500 dark:text-slate-400 font-medium text-[10px] sm:text-sm mt-0.5 sm:mt-1 line-clamp-1">{animal.breed}</p>
            </div>
        ...
        <p className="text-slate-600 dark:text-slate-400 mt-1 sm:mt-4 flex-grow text-[10px] sm:text-sm line-clamp-2 leading-relaxed">{animal.description}</p>
      </div>
