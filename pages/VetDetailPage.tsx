  ...
  const StarRating: React.FC<{ rating: number }> = ({ rating }) => (
    <div className="flex items-center">
      {[...Array(5)].map((_, i) => (
        <svg
          key={i}
          className={`w-5 h-5 ${i < Math.round(rating) ? 'text-yellow-400' : 'text-slate-300 dark:text-slate-600'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="glass-card p-8 flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-8">
            <img
              src={vet.imageUrl}
              alt={vet.name}
              className="w-40 h-40 rounded-full object-cover ring-4 ring-orange-500/30 flex-shrink-0"
            />
            <div>
              ...
              <a
                href={`tel:${vet.phone}`}
                className="inline-block mt-4 bg-slate-200/50 dark:bg-slate-700/50 px-4 py-2 rounded-full font-semibold hover:bg-slate-300/50"
              >
                {vet.phone}
              </a>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
                    ...
                    {vet.reviews.map((review) => (
                      ...
                      >
                        <div className="flex items-center mb-1">
                          ...
                          <p className="ml-3 font-bold text-slate-700 dark:text-slate-200">
                            {review.author}
                          </p>
                        </div>
            ...
            <div className="lg:col-span-1">
              <div className="glass-card p-6 sticky top-24">
                ...
                <p className="text-center text-slate-600 dark:text-slate-300 text-sm mb-5">
                ...
                <div className="text-center">
                  ...
                  <a
                    href={`tel:${vet.phone}`}
                    className="mt-3 inline-block w-full bg-slate-200/50 dark:bg-slate-700/50 px-4 py-2 rounded-full font-semibold hover:bg-slate-300/50"
                  >
                    Call Clinic
                  </a>
                </div>
