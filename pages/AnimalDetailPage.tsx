...
const AnimalDetailPage: React.FC = () => {
  ...
  return (
    <>
        ...
        <div className="glass-card overflow-hidden md:flex">
          ...
          <div className="p-8 md:p-12 md:w-1/2 flex flex-col justify-between">
            <div>
              ...
              <p className="text-xl text-slate-700 dark:text-slate-200 font-semibold mt-2">
                {animal.breed}
              </p>
              <div className="mt-4 text-lg text-slate-600 dark:text-slate-300 flex items-center space-x-4">
                ...
                <span className="text-slate-400 dark:text-slate-500">&bull;</span>
                <span
                  className={`px-2 py-0.5 text-sm rounded-full ${animal.status === 'Available' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}
                >
                  {animal.status}
                </span>
              </div>
              <p className="text-slate-700 dark:text-slate-300 mt-6 leading-relaxed text-lg">
                {animal.description}
              </p>
            </div>
      ...
      <AdoptionForm animal={animal} isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} />
    </>
  );
};
