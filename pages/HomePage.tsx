...
const HomePage: React.FC = () => {
  ...
  return (
    <div className="w-full">
      {/* Hero Section */}
      <section className="container mx-auto px-4 md:px-6 pt-16 md:pt-24 pb-12 animate-fade-in" style={{ animationDelay: '0ms' }}>
        <div className="glass-card text-center p-8 md:p-20 relative overflow-hidden">
          ...
          <h1 className="relative z-10 text-4xl md:text-7xl font-extrabold text-slate-800 dark:text-white drop-shadow-sm tracking-tight leading-tight">{t('hero_title')}</h1>
      ...
      <section className="py-16 md:py-20 animate-fade-in" style={{ animationDelay: '100ms' }}>
      ...
      <section className="py-16 md:py-20 animate-fade-in" style={{ animationDelay: '200ms' }}>
        <div className="container mx-auto px-4 md:px-6">
             <div className="glass-card p-0 overflow-hidden flex flex-col md:flex-row-reverse items-center">
                <div className="md:w-1/2 h-64 md:h-auto w-full">
                    <img src="https://picsum.photos/seed/services-cta/800/600" alt="Professional Pet Services" className="w-full h-full object-cover" loading="lazy" />
                </div>
                <div className="md:w-1/2 p-8 md:p-16 text-center md:text-left">
                    <h2 className="text-3xl md:text-5xl font-bold text-slate-800 dark:text-white leading-tight">{t('section_services_title')}</h2>
                    <p className="text-base md:text-lg text-slate-600 dark:text-slate-300 mt-6 leading-relaxed">
                        {t('section_services_desc')}
                    </p>
                    <Link
                      to="/services"
                      className="mt-8 inline-flex items-center space-x-2 bg-orange-500 text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-orange-600 transition-all transform hover:scale-105 duration-300 shadow-lg"
                    >
      ...
      <section className="py-12 md:py-16 animate-fade-in" style={{ animationDelay: '300ms' }}>
          <div className="container mx-auto px-4 md:px-6 text-center">
              <h2 className="text-2xl font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-10">{t('section_brands')}</h2>
              <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12 opacity-80 hover:opacity-100 transition-opacity">
                  {MOCK_BRANDS.map(brand => (
                      <Link key={brand.id} to="/shop" state={{ brand: brand.name }} className="grayscale hover:grayscale-0 transition-all duration-300 transform hover:scale-110">
                          <img src={brand.logoUrl} alt={brand.name} className="h-10 md:h-14 w-auto object-contain" />
                      </Link>
                  ))}
              </div>
          </div>
      ...
      <section className="py-16 md:py-20 bg-white/50 dark:bg-black/20 backdrop-blur-sm animate-fade-in" style={{ animationDelay: '400ms' }}>
        <div className="container mx-auto px-4 md:px-6 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-slate-800 dark:text-white mb-4">{t('section_new_arrivals')}</h2>
          <p className="max-w-2xl mx-auto text-base md:text-lg text-slate-600 dark:text-slate-400 mb-12">
          ...
          <div className="mt-16">
            <Link
                to="/shop"
                className="inline-block border-2 border-orange-500 text-orange-600 dark:text-orange-400 font-bold py-3 px-10 rounded-full text-lg hover:bg-orange-500 hover:text-white transition-all transform hover:scale-105 duration-300"
            >
                {t('btn_explore_shop')}
            </Link>
          </div>
      ...
      {/* PetBhai+ CTA */}
      <section className="py-16 md:py-20 animate-fade-in" style={{ animationDelay: '500ms' }}>
        <div className="container mx-auto px-4 md:px-6">
          <div className="glass-card text-center p-8 md:p-20 bg-gradient-to-br from-yellow-100/50 to-orange-100/50 dark:from-yellow-900/20 dark:to-orange-900/20 border-orange-200 dark:border-orange-800/30">
            <h2 className="text-3xl md:text-5xl font-extrabold text-slate-800 dark:text-white mb-6">{t('section_plus_title')}</h2>
            <p className="max-w-3xl mx-auto text-lg md:text-xl text-slate-700 dark:text-slate-200 mb-10 leading-relaxed">
              {t('section_plus_desc')}
            </p>
            <Link
              to="/plus-membership"
              className="inline-block bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-bold py-4 px-12 rounded-full text-lg hover:from-yellow-600 hover:to-orange-600 transition-all transform hover:scale-105 duration-300 shadow-xl"
            >
              {t('btn_join_plus')}
            </Link>
          </div>
